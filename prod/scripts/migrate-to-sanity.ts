...
async function migrateSiteSettings() {
  ...
  const doc = {
    ...
    ctas: homeData.ctas?.map(
      (cta: { id: string; label: string; href: string; variant: string }) => ({
        _key: cta.id,
        label: cta.label,
        href: cta.href,
        variant: cta.variant,
    ...
    professionalLinks: connectData.professionalLinks?.map(
      (link: { id: string; type: string; label: string; url: string; icon: string }) => ({
        _key: link.id,
        type: link.type,
        label: link.label,
        url: link.url,
